...
# Average value of 10 times for 10 seconds
#
import time
import dht11
...
def main():
  ...
  # 10 counter
  cnt = 0
  # 10 times avarage <- Temperature
  averageTemp = 0

  # 10 times average <- Humidity
  averageHumi = 0
...
#        time.sleep(60)
    ...
    if cnt > 10:
      wks.update_cell(ln, 1, datetime.now().strftime("%Y/%m/%d %H:%M:%S"))
      print(wks.cell(ln, 1))
      wks.update_cell(ln, 2, averageTemp/10)
      print(wks.cell(ln, 2))
      wks.update_cell(ln, 3, averageHumi/10)
      print(wks.cell(ln, 3))
    ...
    time.sleep(10)
    averageTemp += result.temperature
    averageHumi += result.humidity
    cnt+=1
